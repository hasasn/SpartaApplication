...
func helloWorld(event *sparta.LambdaEvent, context *sparta.LambdaContext, w *http.ResponseWriter, logger *logrus.Logger) {
...
func sampleData() []*sparta.LambdaAWSInfo {
	...
	lambdaFn := sparta.NewLambda(LAMBDA_EXECUTE_ROLE_NAME, helloWorld, nil)
	...
	lambdaFn.EventSourceMappings = append(lambdaFn.EventSourceMappings, &lambda.CreateEventSourceMappingInput{
		EventSourceArn:   aws.String("arn:aws:dynamodb:us-west-2:123412341234:table/tableName/stream/2015-10-22T15:05:13.779"),
	...
	lambdaFunctions = append(lambdaFunctions, sparta.NewLambda(LAMBDA_EXECUTE_ROLE_NAME, logic.LambdaLogic, nil))
